...
def main():
    ...
    ap.add_argument("--input-dir", required=True, help="Videos directory")
    ap.add_argument("--pattern", default="**/*.mp4", help="search pattern (glob)")
    ap.add_argument("--rel-prefix", default="dataset", help="rel-dir prefix for CVAT share")
    ...
    ap.add_argument("--skip-existing", help="Skip if tracks/<clip>.json already exists")
    ap.add_argument("--script", default="script.py",
                    help="Path to image extraction and pose-tracking script")
    args = ap.parse_args()

    input_dir = Path(args.input_dir).expanduser().resolve()
    videos = sorted(input_dir.glob(args.pattern))
    if not videos:
        print(f"No videos found with pattern {args.pattern} in {input_dir}", file=sys.stderr)
        sys.exit(1)
